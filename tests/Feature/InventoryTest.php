...
class InventoryTest extends TestCase
{
    ...
    /**
     * Seeds ingredients, recipes and box orders
     *
     * @return this
     */
    public function seedAllData($date)
    {
     ...
     * @return void
     */
    public function testPurchaseOrderApiWithDateFilter()
    {
        ...
        // Seed all data
        $this->seedAllData($date);

        // Make api call to list all ingredients across all suppliers
        $response = $this->get(
            "/api/purchase-order?order_date={$date->format('Y-m-d')}"
        );
     ...
     * API should return the list of ingredients to be purchased
     * to fulfill all orders for the 7 days from the given date
     * and filtered by the given supplier id
     *
     * @return void
     */
    public function testPurchaseOrderApiWithSupplierAndDateFilter()
    {
        // We'll take today's date.
        // Any date will work here, since all box delivery_dates
        // are set relative to this date.
        ...
        // Make api call to list all ingredients for supplier_id 1
        $response = $this->get(
            "/api/purchase-order?order_date={$date->format(
                'Y-m-d'
            )}&supplier_id=1"
        );
        $response->assertStatus(200);
         ...
         * 3. Multiply this number by 2 since every ingredient's amount in
         *    every recipe is hardcoded to 2.
         * 4. Only the first 5 ingredients are supplied by supplier_id 1,
         *    so no other ingredients should be present in the response.
         * 5. Validate these numbers in the response.
         */
        $response->assertJson([
            [
                'id' => '1',
